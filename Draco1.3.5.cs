...
namespace Draco
{
    class DracoBeta
    {
        ...
        public static DataTable ConvertCsvToDataTable(string fileName)
        {
            ...
            using (StreamReader sr = new StreamReader("./awa.csv"))
            {
...
";
            ...
            api.addBeforeActListener(EventKey.onRespawn, x =>
             {
                 ...
                 else
                 {
                     ...
                     int ti = 0;
                     if (level[a.playername] < 50 & level[a.playername] >= 20) { ti = 1; }
                     if (level[a.playername] > 50) { ti = 2; }
            ...
            api.addBeforeActListener(EventKey.onUseItem, x =>
            {
                ...
                {
                    int ti = 5;
                    if (level[a.playername] < 50 & level[a.playername] >= 20) { ti = 10; }
                    if (level[a.playername] > 50) { ti = 15; }
                    if(b > 500)
                    {
...
namespace CSR
{
    partial class Plugin
    {
        ...
        public static void onStart(MCCSAPI api)
        {
            ...
            Console.WriteLine("[Draco]您正在使用Draco1.3.5");
